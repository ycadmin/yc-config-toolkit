...
public class VersionManagedBean implements IObserver, Serializable {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(VersionManagedBean.class);
	...
	public void createVersion() {
		...
		if (!Strings.isNullOrEmpty(versionToBeCreated)) {
			...
			boolean suc = nodeService.createProperty(ZKPaths.makePath(
					nodeAuth.getAuthedNode(), versionToBeCreated), null);
			if (suc) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage("版本已创建.", versionToBeCreated));
				...
				FacesContext.getCurrentInstance().addMessage(
						...
						new FacesMessage(FacesMessage.SEVERITY_ERROR,
								"版本创建失败.", versionToBeCreated));
			}
		}
	}

	public void deleteVersion() {
		if (selectedVersion != null && !selectedVersion.equals("")) {
			if (LOGGER.isInfoEnabled()) {
				LOGGER.info("Delete version [{}].", selectedVersion);
			}
			String versionPathString = ZKPaths.makePath(
					nodeAuth.getAuthedNode(), selectedVersion);
			nodeService.deleteProperty(versionPathString);
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage("当前版本已删除.", versionPathString));
			selectedVersion = null;
			refresh();
			if (LOGGER.isInfoEnabled()) {
				LOGGER.info("Delete version [{}] success.", selectedVersion);
			}
		}
		else {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage("请选择要删除的版本."));
		}
	}
	...
	public void cloneVersion() {
		...
		if (!Strings.isNullOrEmpty(versionToClone)
				&& !Strings.isNullOrEmpty(selectedVersion)) {

			if (LOGGER.isInfoEnabled()) {
				LOGGER.info("Clone version [{}] from version [{}].",
						versionToClone, selectedVersion);
			}
			cloneTree(
					ZKPaths.makePath(nodeAuth.getAuthedNode(), selectedVersion),
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage("版本已克隆.", versionToClone));
			refresh();
	...
	private void cloneTree(String sourceVersionPath,
			String destinationVersionPath) {
		List<String> sourceGroups = nodeService.listChildren(sourceVersionPath);
		if (sourceGroups != null) {
			for (String sourceGroup : sourceGroups) {
				String sourceGroupFullPath = ZKPaths.makePath(
						sourceVersionPath, sourceGroup);
				String destinationGroupFullPath = ZKPaths.makePath(
						destinationVersionPath, sourceGroup);

				nodeService.createProperty(destinationGroupFullPath, null);
				List<PropertyItem> sourceProperties = nodeService
						.findProperties(sourceGroupFullPath);
				if (sourceProperties != null) {
					for (PropertyItem sourceProperty : sourceProperties) {
						nodeService.createProperty(ZKPaths.makePath(
								destinationGroupFullPath,
								sourceProperty.getName()), sourceProperty
								.getValue());
					}
				}
			}
	...
	private void refresh() {
		...
		if (!Strings.isNullOrEmpty(rootNode)) {
			...
			if (children != null) {
				children = Lists.newArrayList(Iterables.filter(children,
