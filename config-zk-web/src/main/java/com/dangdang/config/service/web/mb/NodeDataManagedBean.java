...
public class NodeDataManagedBean implements Serializable, IObserver {
	...
	public void onPropertyEdit(RowEditEvent event) {
		...
		if (suc) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("属性保存成功.", name));
		} else {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "属性保存失败.", name));
		}
	...
	public void createProperty() {
		...
		if (created) {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("属性已创建.", propPath));
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "属性创建失败.", propPath));
		}
	}
	...
	public void deleteProperty(String propName) {
		...
		FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("属性已删除.", propPath));
		refreshNodeProperties(selectedNode);
	}
	...
	private boolean checkPropertyGroupCheckedStatus() {
		...
		if (notChecked) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("请先选择一个属性组."));
		}
		return !notChecked;
	}
