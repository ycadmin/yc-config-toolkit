...
public class PropertyGroupManagedBean implements Serializable, IObserver {
	...
	public void createNode() {
		...
		if (created) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("属性组已创建。", newPropertyGroupName));
			refreshGroup();
			newPropertyGroup.setValue(null);
			nodeData.refreshNodeProperties(null);
		} else {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "属性组创建失败。", newPropertyGroupName));
		}
	}
	...
	public void deleteNode(String propertyGroup) {
		...
		FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("属性组已删除。", propertyGroup));
		refreshGroup();
	}
	...
	public void propertyGroupUpload(FileUploadEvent event) {
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "文件校验错误！", fileName));
	...
	private void savePropertyGroup(String fileName, String group, InputStream inputstream) throws IOException {
		...
		if (!items.isEmpty()) {
			...
			if (created) {
				...
				refreshGroup();
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("成功！", fileName + " 已上传。"));
			} else {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, "创建组文件失败.", fileName));
			}
		} else {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "空文件.", fileName));
		}
	}
	...
	public void propertyZipUpload(FileUploadEvent event) {
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "上传文件错误.", fileName));
	...
	private void savePropertyGroupOld(String fileName, String group, InputStream inputstream) throws IOException {
		...
		if (!properties.isEmpty()) {
			...
			if (created) {
				...
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, "创建组属性文件失败.", fileName));
			}
		} else {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "空文件.", fileName));
		}
